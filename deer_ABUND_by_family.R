...
abundance_data <- abundance_raw_data[1:141,] %>%
...
effect_sizes_abundance <- escalc("SMD", # Specify the outcome that we are measuing, RD, RR, OR, SMD etc.
                                 n1i = abundance_data$sample_size_t, # Then, follow with all of the columns needed to compute SMD
                                 n2i = abundance_data$sample_size_c, 
                                 m1i = abundance_data$mean_t, 
                                 m2i = abundance_data$mean_c, 
                                 sd1i = abundance_data$SD_t, 
                                 sd2i = abundance_data$SD_c,
                                 data = abundance_data)
...
forest(mixed_effects_abund_family)
